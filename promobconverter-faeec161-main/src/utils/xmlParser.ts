...
export const parsePromobXMLComplete = (xmlContent: string): ParsedPiece[] => {
  try {
    ...
    allItems.forEach(item => {
      ...
      // Pular itens de estrutura (Armário, Caixa, etc.)
      const lowerDesc = description.toLowerCase();
      if (lowerDesc.includes("armário") || lowerDesc.includes("caixa") || 
          lowerDesc.includes("gaveteiro") || lowerDesc.includes("dispenseiro") ||
          lowerDesc.includes("acessório") || lowerDesc.includes("ferragem")) {
        return;
      }
...
const unifyIdenticalPieces = (pieces: ParsedPiece[]): ParsedPiece[] => {
  ...
  pieces.forEach(piece => {
    ...
    const key = `${piece.description}_${piece.material}_${piece.model}_${piece.thickness}_${piece.width}_${piece.height}_${piece.depth}`;
...
const processItemElementsOptimized = (itemElements: NodeListOf<Element>, csvContent: string): string => {
  ...
  try {
    ...
    itemArray.forEach(item => {
      // MUDANÇA: Aceitar tanto UNIQUEID quanto ID para suportar peças avulsas
      const uniqueId = item.getAttribute("UNIQUEID") || item.getAttribute("ID") || "";
      ...
      if (!uniqueId) {
        console.warn("Item encontrado sem UNIQUEID ou ID - será ignorado", description);
        return false;
      }
